...
//                 {
//                     setMessage("The Bike List is Empty")
//                 }
//                 else{
//                     getBikeData();
//                 }
...
function Cart() {
  ...
  function getBikeData() {
    axios
      .get(`http://localhost:8080/users/customer/cartBikeList/${userId}`)
      .then((response) => {
        if (response) setBikesList(response.data);
      })
      .catch((err) => {
        console.log(err);
      });
  ...
  function getPartData() {
    axios
      .get(`http://localhost:8080/users/customer/cartPartList/${userId}`)
      .then((response) => {
        setPartsList(response.data);
      })
      .catch((err) => {
  ...
  function calculateTotalPrice() {
    let bikeTotal = bikes.reduce(
      (acc, bike) => acc + bike.price * bike.bikeQuantity,
      0
    );
    let partTotal = parts.reduce(
      (acc, part) => acc + part.price * part.partQuantity,
      0
    );
    setTotalPrice(bikeTotal + partTotal);
  }
  ...
  const deleteBike = (cartId) => {
    axios
      .post(`http://localhost:8080/users/customer/removeBikeFromCart/${cartId}`)
      .then((response) => {
        if (response.status === 500) {
          setMessage("The Bike List is Empty");
        } else {
  ...
  const deletePart = (cartId) => {
    axios
      .post(`http://localhost:8080/users/customer/removePartFromCart/${cartId}`)
      .then((response) => {
        if (response.status === 500) {
          setMessage("The Bike List is Empty");
        } else {
  ...
  const Order =()=>
  {
    axios.post(`http://localhost:8080/users/customer/order/${userId}`).
    then((response) => {
  ...
  return (
    <div className="container">
      ...
      <div>
        <Link>
          <Button className="btn btn-info" onClick={()=>{Order()}} >Order</Button>
        </Link>
      </div>
